...
do
 ...
 #../svm_learn -t 1 -d 5 $TRAIN_FILE $MODEL_FILE > $TEMP_FILE
 ...
 echo ""  >> $RESULT_FILE
 echo "linear SVM with default parameters" >> $RESULT_FILE
 ../svm_learn  $TRAIN_FILE $MODEL_FILE > $TEMP_FILE
 ../svm_classify -v 1 $TEST_FILE $MODEL_FILE $PRED_FILE > $TEMP_RESULT
 cat $TEMP_RESULT >> $RESULT_FILE
 sed '/^Reading model/d' $TEMP_RESULT > $TEMP_RESULT"1"
 sed '/^Precision/d' $TEMP_RESULT"1" > $TEMP_RESULT
 sed "$SVM_PATTERN$k linear $g \1/g" $TEMP_RESULT >> "better"$RESULT_FILE
